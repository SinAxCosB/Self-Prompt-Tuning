...
def prompt_generator(model, args, model_depth=12, sample_size=1):
    # dataset
    ...
    if args.prompt_deep:
        assert args.prompt_sample_type in ["random", "mean_pooling", "max_pooling"] 
        ...
        if args.prompt_sample_type == "random":
            ...
            # v2 window_size = length // num_prompts
            # prompt sample form batch * num_prompts
            prompt = []
            for i in range(model.depth):
                pro = []
                B, L, D = x[i].shape
...
def create_model(args, num_class):
    vpt = ['vpt_vit_base_patch16', 'vpt_vit_large_patch16', 'vpt_vit_huge_patch14']
    ...
    pretrain_model = Models.__dict__['vit_base_patch16']()
    ...
    elif args.tuning_type in ["fine-tuning"]:
        model = Models.__dict__['vit_base_patch16'](
            drop_path_rate=args.drop_path)
...
if __name__ == '__main__':  
    ...
    parser.add_argument("--model_weights", type=str, default="", help="load trained weights for test.")
    ...
    parser.add_argument("--prompt_sample_type", type=str, default='random', choices=["random", "mean_pooling", "max_pooling"] )

    # * MoCo pretrain
    parser.add_argument('--finetune', type=str, default='./ckpt/MoCo/vit-b-300ep.pth', help='pretrained weights')
    ...
    if args.model_weights:   
        train_set, test_set, num_class = Dataset.__dict__[args.dataset]()

        model = timm.create_model(args.model_name, drop_path_rate=args.drop_path, pretrained=False)
        # modify the output dimension (num classes) according to the task
        in_features = model.head.in_features
        model.head = torch.nn.Linear(in_features=in_features, out_features=num_class)

        # load fine-tuned ckpt to evaluate the accuracy on the validation / test set
        print('Load fine-tuned weights for {}'.format(args.model_name))
        ...
        acc1, acc5 = testmodel(model=model, test_data=test_set, args=args)
